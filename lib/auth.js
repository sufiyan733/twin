...
function isProduction() {
  ...
  return baseUrl ? baseUrl.startsWith("https://") : false;
}
...
export function getAuth() {
  if (!cachedAuth) {
    cachedAuth = betterAuth({
      ...
      advanced: {
        // Keep cookie names consistent between HTTP (dev) and HTTPS (prod).
        // Without a fixed prefix the browser silently prepends __Secure- on
        // HTTPS, so the server reads the unprefixed name and gets nothing.
        cookiePrefix: "better-auth",
        // Only set Secure flag on real HTTPS deployments.
