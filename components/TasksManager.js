...
function ExpandPanel({ open, children }) {
  ...
  useEffect(() => {
    ...
    if (open) {
      // measure then animate to exact content height
  ...
  return (
    <div
      style={{ height, overflow: "hidden", transition: "height 320ms cubic-bezier(0.4,0,0.2,1)" }}
    >
      <div ref={ref}>
        {children}
      </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/5 bg-[#030818] shrink-0">
          ...
          <button
            onClick={onClose}
            className="flex items-center justify-center h-9 w-9 rounded-full bg-white/[0.04] border border-white/[0.06] text-white/60 hover:bg-white/10 hover:text-white transition-all duration-200 active:scale-90"
          >
          ...
          <div className="relative min-w-[130px]">
            {isEditingTime ? (
              ...
              <button
                ...
                className="w-full group relative bg-white/[0.04] border border-white/10 hover:border-[#00d0ff]/40 rounded-xl px-3 py-2 text-sm text-white hover:text-[#00d0ff] font-bold transition-all duration-200 text-center flex items-center justify-center gap-1.5"
              >
                <Clock size={14} className="text-[#00d0ff] group-hover:scale-110 transition-transform duration-200" />
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin scrollbar-thumb-[#00d0ff]/20 scrollbar-track-transparent">
          {tasks.length === 0 ? (
            ...
            tasks.map((task, i) => {
              ...
              return (
                <div
                  key={task.id}
                  className={`rounded-[16px] border bg-[#07112c]/60 overflow-hidden transition-all duration-300 ${
                  ...
                  style={{
                    // stagger entrance
                    animation: `fadeSlideUp 300ms ${i * 40}ms both ease-out`,
                  }}
                >
                  {/* Task Summary Row */}
                  <div
                    className="flex items-center gap-3 p-3 cursor-pointer hover:bg-white/[0.02] transition-colors duration-150 select-none"
                    onClick={() => setExpandedTaskId(isExpanded ? null : task.id)}
                  >
                    <div className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[#0a1535] border border-[#00d0ff]/20 shadow-inner transition-all duration-200 ${task.checked ? "opacity-50 scale-90" : "scale-100"}`}>
                      {task.icon && <task.icon size={16} className="text-[#00d0ff] drop-shadow-[0_0_5px_rgba(0,208,255,0.5)]" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className={`text-sm font-semibold truncate transition-all duration-200 ${task.checked ? "text-white/40 line-through decoration-[#00d0ff]/60" : "text-white"}`}>
                        {task.title}
                      </h3>
                    ...
                    <div className={`text-[#00d0ff] opacity-60 shrink-0 transition-transform duration-300 ${isExpanded ? "rotate-180" : "rotate-0"}`}>
                      <ChevronDown size={18} />
                    </div>
                  </div>
                    ...
                    <div className="p-4 pt-0 border-t border-white/5 space-y-4 mt-2">
                      <div className="grid grid-cols-1 gap-3 mt-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full rounded-xl bg-white/[0.04] border border-white/10 px-3 py-2 text-sm text-white outline-none focus:border-[#00d0ff]/50 transition-all duration-200"
                          />
                        </div>
                        <div>
                      ...
                      <div className="pt-2">
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-all duration-200 text-xs font-semibold"
                        >
                          <Trash2 size={14} />
                          Delete Task
                        </button>
          ...
          <button
            onClick={addNewTask}
            className="w-full flex items-center justify-center gap-2 py-3 mt-4 rounded-xl border border-dashed border-[#00d0ff]/40 text-[#00d0ff] hover:bg-[#00d0ff]/10 hover:border-[#00d0ff]/60 transition-all duration-200 text-sm font-semibold active:scale-[0.98]"
          >
            <Plus size={16} />
            Add New Task
          </button>
      ...
      {/* Keyframe for staggered task cards entrance */}
      <style>{`
